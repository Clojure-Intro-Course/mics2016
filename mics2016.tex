...
\section{Languages}\label{sec:lang}
	...
	Lisp is a family of programming languages that dates from 1958, developed by John McCarthy based on Alonzo Church's lambda calculus. 
	%\emcomment{Lisp is definitely based on the lambda calculus, but it wasn't created from it. In particular, lambda calculus is call by name (in its classical form)}
	 Lisp was the first language where the internal representation of the program was in the same data structures the program was written in. \hfcomment{???}
\emcomment{where data and program code were internally stored using the same data structure (lists) which opened possibilities for manipulating not just data, but also program code, within a program.}
\emcomment{Mention that Lisps are functional}
...
Lists in Lisp are commonly created by using the list function
	...
	-> '(2 4 6 8)
	\end{verbatim}
The return value \texttt{'(2 4 6 8)} uses the single quote \texttt{'} to indicate that the expression should not be evaluated (doing so would attempt to apply the number \texttt{2} as a function), but instead it should be treated as list \emcomment{as data storage?} \hfcomment{wrong, but close enough to the truth?} \emcomment{yes}	
	\begin{verbatim}
	(list 2 4 6 8)
	-> '(2 4 6 8)
	\end{verbatim}
	...
	\emcomment{To be fair, though, they weren't the first ones to use it (SICP was the prior approach) NEED TO REFERENCE SICP}:
...
Racket is designed for introductory students, \emcomment{If we just say this about Racket, we would have a really hard time arguing for Clojure. Perhaps saying that Racket features a set of ``layered'' languages that introduce beginner students to a subset of Lisp?} 
while Clojure focuses on concurrency and integration with the Java virtual machine. 
...
An example of a hashmap using keywords follows: \emcomment{say something about the first example below; wrong comment?}
...
\begin{verbatim}
	...
	-> #<position>
\end{verbatim}
Structs automatically generate an accessor function for each field in the structure. \emcomment{expplain syntax; mention that structs aren't extensible}
\begin{verbatim}
...
Participants will not be allowed to consult outside sources, the reviewer, or other participants during this period of the study except for the language's public API. \emcomment{clojuredocs  aren't exactly a public API, would this be allowed?} 
...
\emcomment{The part that follows should really be condensed a lot}
...
\emcomment{Perhaps labeling them with the level? C1-2 for Clojure level 1, q. 2?}
...
If we did not remove the pair, our participant could end up with question R1 in their Racket test and C1 in their Clojure test, effectively giving them the same questions twice. \emcomment{The previous two sentences should be combined} 
...
The website randomly select three questions out of the remaining four that have not been used and puts them into the Clojure Test. \emcomment{No, all of the remaining questions will be used in the test in Racket}
