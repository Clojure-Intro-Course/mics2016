...
\section{Languages}\label{sec:lang}
	...
	Lisp was the first where data and program code were internally stored using the same data structure which opened possibilities for manipulating not just data, but also program code, within a program.
...
Racket and Clojure, while still Lisps, satisfy different design goals. 
Racket features a set of languages with limited subsets of the full language that allow instructors to slowly introduce features to students.Clojure focuses on concurrency and integration with the Java virtual machine. 
